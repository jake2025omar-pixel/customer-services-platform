...
type Campaign = { id: string; name: string; prize: string; startsAt: string | Date; endsAt: string | Date; eligibility: string; officialRules: string; winnerSelection: string; prizeDelivery: string; ticketCost: number; participantCount: number; userJoined: boolean };
...
export default function Contests() {
  ...
  const { data: dashboard } = trpc.platform.dashboard.useQuery();
  ...
  useEffect(() => { loadCampaigns().catch(error => setMessage(error instanceof Error ? error.message : "Unable to load campaigns")).finally(() => setLoading(false)); }, []);

  async function join(campaignId: string) {
    setMessage(null); setJoining(campaignId);
    try {
      const response = await fetch(`/api/campaigns/${campaignId}/join`, { method: "POST", headers: { "content-type": "application/json", "Idempotency-Key": crypto.randomUUID() }, credentials: "include", body: "{}" });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Unable to join campaign");
      await loadCampaigns();
      setMessage(data.status === "idempotent_replay" ? "This join request was already processed." : data.status === "already_joined" ? "You have already joined this campaign." : "You joined the campaign and 50 tickets were deducted.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to join campaign"); } finally { setJoining(null); }
  }
